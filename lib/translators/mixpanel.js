...
module.exports = function(event, parse) {
  ...
  var fn = translators[event.event]

  if (!fn) throw new Error('missing event translator ' + event.event);

  fn(event, o);

  if (o.unstruct_event) o.unstruct_event = JSON.stringify(o.unstruct_event);
...
function schema(namespace, event, version) {
  ...
  return 'iglu:com.boombox.' + namespace + '/' + event + '/jsonschema/' + version;
}
